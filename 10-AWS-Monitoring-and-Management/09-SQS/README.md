...
**Queue Types:**
- **Standard Queue**: At-least-once delivery, best-effort ordering, high throughput (unlimited TPS).
- **FIFO Queue**: Exactly-once processing(no duplicates at queue level), strict order preserved. Limited to 300 TPS(3000 with batching). Ordered by message group ID, not across groups. Queue name must end with **.fifo**.
